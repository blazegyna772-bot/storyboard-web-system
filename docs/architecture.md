...
输入：当前场剧本 + 当前单集概要 + 可选当前场次概要 + 资产索引。

输出：场级底座、15 秒内视频块、剧本原文、块任务、起止状态、资产引用和临时资产。

这是当前分镜统筹层的生产分块节点，不生成最终视频提示词。

### 视频提示词

输入：当前分块规划 + 资产索引 + 可选上一块视频提示词。

输出：按视频块装配的提示词草案。参考图路径、草稿状态和视频路径由后端程序字段维护。
...
- 分块规划：分块边界、剧本原文、块任务、起止状态、资产引用错误时重跑。
- 视频提示词：视频模型语言、参考图、模型参数错误时重跑。

视频提示词支持整场、当前块和区间三种入口；分块规划仍按场执行。

## 资产衔接原则

最终生产状态：

- 分块规划/视频提示词 必须引用资产真源 ID、版本和真源图片。
- LLM 不允许临时发明资产名称。
...
- 分块规划和视频提示词已读取资产索引。
- LLM 只使用资产 ID、名称和版本标签，不读取长外观描述。
- 最终视频生成接入后，由后端按资产 ID 和版本标签查找参考图。
